 ...
 * {@code Config.effectiveEdgeCost(edge)} — the single project-wide
 * effective-cost formula ({@link Graph.Edge#effectiveCost()}, never
 * recomputed here) plus the index-derived ROUTE_PENALTY on flood-prone
 * roads — so a "fastest route under weighted, degraded roads" is exactly
 * what this returns. Closed roads (`edge.isClosed`) are skipped entirely,
 * matching how A3's DFS already treats the same Graph interface.
 *
    ...
    private static Result run(Graph graph, String source, StringBuilder trace) {
        ...
        while (!frontier.isEmpty()) {
            ...
            for (Graph.Edge e : neighbors) {
                ...
                double weight = Config.effectiveEdgeCost(e);
